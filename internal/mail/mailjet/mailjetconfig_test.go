...
func Test_Integration_createConfig(t *testing.T) {
	...
	if err := os.Setenv(apiKeyPublicEnvKey, publicKey); err != nil {
		t.Fatalf("Failed to set environment variable: %v", err)
	}
	defer func() {
		if err := os.Unsetenv(apiKeyPublicEnvKey); err != nil {
			t.Fatalf("Failed to unset environment variable: %v", err)
		}
	}()
	privateKey := "test-private-key"
	if err := os.Setenv(apiKeyPrivateEnvKey, privateKey); err != nil {
		t.Fatalf("Failed to set environment variable: %v", err)
	}
	defer func() {
		if err := os.Unsetenv(apiKeyPrivateEnvKey); err != nil {
			t.Fatalf("Failed to unset environment variable: %v", err)
		}
	}()
